...
class SearchOutcome:
    ...
    scanned_chunks: int
    elapsed_ms: float
    # 这次实际走的是哪条路 —— keyword（v5）/ semantic（v6）/ 降级
    mode: str = "keyword"
...
class KnowledgeBase:
    ...
    def __init__(
        ...
        *,
        ...
        # 检索模式："keyword"（v5 的字面匹配）| "semantic"（v6 的向量）
        # ⚠️ semantic 模式下如果向量算不出来，会**降级**成 keyword ——
    ...
    def search(self, query: str, *, top_k: int = 5) -> SearchOutcome:
        ...
        if self.mode == "semantic" and self.embedder is not None and self.cache is not None:
            try:
        ...
        # 注意：semantic 模式下「0 命中」是**合法结果**，不偷偷换成关键词 ——
    ...
    def describe(self) -> str:
        ...
        if self.mode == "semantic" and self.embedder is not None:
        ...
        if self.mode == "semantic" and self.cache is not None and self.embedder is not None:
            lines.append(
