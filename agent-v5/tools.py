...
try:
    KNOWLEDGE = KnowledgeBase(NOTES_DIR, chunk_max_chars=CHUNK_MAX_CHARS)
except Exception as _exc:  # 理论上不会发生，但工具层不该因为“库”启不来就整程序崩
    KNOWLEDGE = None  # type: ignore[assignment]
    _KNOWLEDGE_ERROR = f"{type(_exc).__name__}: {_exc}"
else:
    _KNOWLEDGE_ERROR = None
...
def search_notes(query: str, limit: int = RETRIEVAL_TOP_K) -> str:
    ...
    if KNOWLEDGE is None:
        return f"检索功能不可用（{_KNOWLEDGE_ERROR}）"
    ...
    # ── 给用户看的（不占模型上下文）────────────────────────────
    if outcome.hits:
        scores = " / ".join(f"{h.score:.2f}" for h in outcome.hits[:3])
        console.print(
            f"  [cyan]🔍 检索[/cyan] [bold]{query}[/bold] → "
            f"[green]{len(outcome.hits)} 段命中[/green]"
            f" [dim]（最高分 {scores} · 扫描 {outcome.scanned_chunks} 块 · "
            f"{outcome.elapsed_ms:.1f} ms）[/dim]"
        )
        for hit in outcome.hits:
            terms = "、".join(hit.matched[:4]) or "（标题命中）"
            console.print(f"      [dim]{hit.chunk.citation()} · 命中 {terms}[/dim]")
    else:
        console.print(
            f"  [cyan]🔍 检索[/cyan] [bold]{query}[/bold] → [yellow]0 段命中[/yellow]"
            f" [dim]（扫描 {outcome.scanned_chunks} 块 · {outcome.elapsed_ms:.1f} ms）[/dim]"
        )
        # 这一行是给用户的提醒，也是这一版最怕的失效模式：
        # 检索空手而归时，模型很可能开始编 —— 提醒用户盯住它。
        console.print(
