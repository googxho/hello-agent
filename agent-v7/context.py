...
class ContextManager:
    ...
    def export(self) -> tuple[list[dict[str, Any]], str | None]:
        """导出可存盘的状态：(消息列表, 摘要文本)。

        两样东西被刻意排除在外：

          · **system 提示词** —— 它属于「代码」，不属于「会话」。
            这样以后改了提示词，老会话也能用上新版的
          · **摘要那条消息** —— 它单独存成 summary 字段，
            恢复时按当前格式重新拼（连头尾标记也跟着代码走）

        结果就是：磁盘上只有「用户和助手之间真实发生过的事」。
        """
        keep: list[dict[str, Any]] = []
        for msg in self.messages:
            if msg is self.messages[0]:
                continue
            if self._summary_message is not None and msg is self._summary_message:
                continue
            keep.append(msg)
        return keep, self.summary_text

    def restore(self, messages: list[dict[str, Any]], summary: str | None) -> list[str]:
        """从存盘状态恢复。

        返回**问题列表**：空列表 = 恢复成功。
        有问题就保持原状、一字不动 —— 调用方可以选择「从零开始」。

        ⚠️ 为什么偏偏要在「恢复的这一刻」校验结构？
           如果带着坏结构（比如孤儿 tool 消息）启动，程序不会立刻报错，
           而是在**第一次请求模型时**才炸 —— 到那时，你早就忘了
           「这场对话是从磁盘读来的」。宁可在门口就拦住。
        """
        ...
        if summary:
            ...
            candidate.insert(1, summary_message)
        ...
        self.summary_text = summary or None
    ...
    def _summary_content(self, text: str | None = None) -> str:
        """拼出摘要消息的完整正文（带头尾标记）。

        text 留空就用当前摘要；恢复会话时用它拼「还没装上的」那一条。
        """
        body = self.summary_text if text is None else text
