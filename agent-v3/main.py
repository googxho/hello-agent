"""
命令行入口 —— 交互界面、/summary 命令，以及不需要 API Key 的自检。

    python main.py              # 进入交互对话
    python main.py --selftest   # 测工具 + 测剪枝 + 测压缩（不需要 API Key，不花钱）
...
def _banner() -> Panel:
    ...
    lines = [
        "[bold]hello-agent[/bold] v3 · 会「压缩记忆」的最小 Agent",
    ...
    lines += [
        "",
        "[dim]/context 看上下文占用    /summary 看摘要原文    /reset 清空历史    /exit 退出[/dim]",
    ]
