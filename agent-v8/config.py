...
# ── 结构化输出（v8 的核心，新增）────────────────────────────────────
# v7 的收尾卡在「评估的第一步要程序化判定」，而模型的输出是自由文本。
# v8 让它通过 submit_result「交卷」：answer / found / sources / confidence。
# 校验不过 → 错误回灌 → 重试；试满就放弃（不会假装成功）。
#
# 提交上限：第 N 次提交仍不合规就放弃（默认 3 = 最多交 3 次卷）。
#
# 想亲眼看到「放弃」长什么样？把上限调成 1 —— 模型几乎没有改错的机会：
#
#     LLM_STRUCTURED_MAX_ATTEMPTS=1 python main.py --ask "…" --json
STRUCTURED_MAX_ATTEMPTS = _read_int("LLM_STRUCTURED_MAX_ATTEMPTS", 3)
...
#   2. 给未来的你当文档 —— 一眼看出哪些是共享的、哪些是某个版本独有的
#
...
CONFIG_KEYS: list[tuple[str, str, str]] = [
    ...
    ("LLM_STRUCTURED_MAX_ATTEMPTS", str(STRUCTURED_MAX_ATTEMPTS), "v8 新增"),
